...
/**
 * Unit tests for {@link SmartWellKnownFilter}.
 *
 * <p>Verifies that the filter intercepts {@code GET /fhir/.well-known/smart-configuration},
 * writes the SMART discovery document to the response, and does not continue the filter chain.
 */
class SmartWellKnownFilterTest {

	private SmartWellKnownFilter filter;
    ...
    void setUp() {
        ...
        properties.setAuthorizationServerUrl("http://keycloak.example.org/realms/fhir");
		 ...
		 SmartConfigController controller = new SmartConfigController(properties);
        filter = new SmartWellKnownFilter(controller, objectMapper);
    }
    ...
    class Interception {
        ...
        void handlesGetRequest() throws Exception {
            HttpServletRequest request = mockRequest("GET", SmartWellKnownFilter.WELL_KNOWN_PATH, "");
        ...
        void handlesOptionsRequest() throws Exception {
            HttpServletRequest request = mockRequest("OPTIONS", SmartWellKnownFilter.WELL_KNOWN_PATH, "");
            StringWriter body = new StringWriter();
            HttpServletResponse response = mockResponse(body);
            FilterChain chain = mock(FilterChain.class);
    ...
    private HttpServletRequest mockRequest(final String method, final String uri, final String ctxPath) {
